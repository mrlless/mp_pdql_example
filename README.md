...
select(@Host, Host.OsName as OsName, Host.@Vulners as Vulners, Host.@Vulners.SeverityRating as Severity,Host.@Vulners.VulnerableEntity.Name, Host.@Vulners.Status  ) | filter(Vulners and Host.@Vulners.Status != 'fixed' ) | sort(Severity DESC, Vulners ASC) | group(@Host, OsName, COUNT(*) as TotalVulners) | join(select(@Host, Host.@Vulners as Vulners, Host.@Vulners.SeverityRating as Severity)
