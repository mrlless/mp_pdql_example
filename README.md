...
select(@Host, Host.OsName as OsName, Host.@Vulners as Vulners, Host.@Vulners.SeverityRating as Severity, Host.@Vulners.VulnerableEntity.Name, Host.@Vulners.Status, host.@Vulners.Metrics.HasFix) | filter(Vulners and Host.@Vulners.Status != 'fixed' and Host.@Vulners.Metrics.HasFix = true) | sort(Severity DESC, Vulners ASC) | group(@Host, OsName, COUNT(*) as TotalVulners) | join(select(@Host, Host.@Vulners as Vulners, Host.@Vulners.SeverityRating as Severity,  Host.@Vulners.Status,Host.@Vulners.Metrics.HasFix ) | filter( Host.@Vulners.Status != 'fixed' and Host.@Vulners.Metrics.HasFix = true)
